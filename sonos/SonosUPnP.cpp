...
{
  char stateBuf[32];
  uint8_t state = getState(speakerIP, stateBuf);
  Serial.printf("togglePause: transport state='%s' (%u)\n", stateBuf, state);
  if (state == SONOS_STATE_PLAYING)
  {
    Serial.println("togglePause -> calling Pause");
    pause(speakerIP);
  }
  else
  {
    Serial.println("togglePause -> calling Play");
    play(speakerIP);
  }
}
...
bool SonosUPnP::upnpGetzp(IPAddress ip) // JV new - simple GET status/zp command 
{
  ...
  // Read available response (limit to reasonable size) and print for debugging
  {
    String resp = "";
    uint32_t readStart = millis();
    while (millis() - readStart < 500) {
      while (ethClient.available()) {
    ...
    if (resp.length() > 0) {
      // print first 1024 chars to avoid flooding
      if (resp.length() > 1024) resp = resp.substring(0, 1024) + "\n... (truncated)";
      Serial.println("Sonos UPnP response:");
      Serial.println(resp);
    } else {
      Serial.println("Sonos UPnP response: <no data>");
    }
  }
  return true;
}
...
bool SonosUPnP::upnpPost(IPAddress ip, uint8_t upnpMessageType, PGM_P action_P, const char *field, const char *valueA, const char *valueB, PGM_P extraStart_P, PGM_P extraEnd_P, const char *extraValue)
{
  ...
  // Debug: print target IP/port/service/action
  {
    char actionBuf[64] = "";
    char serviceBuf[64] = "";
    PGM_P svc = getUpnpService(upnpMessageType);
    size_t aLen = strlen_P(action_P);
    if (aLen >= sizeof(actionBuf)) aLen = sizeof(actionBuf) - 1;
    if (aLen) memcpy(actionBuf, (const char*)action_P, aLen);
    actionBuf[aLen] = '\0';
    size_t sLen = strlen_P(svc);
    if (sLen >= sizeof(serviceBuf)) sLen = sizeof(serviceBuf) - 1;
    if (sLen) memcpy(serviceBuf, (const char*)svc, sLen);
    serviceBuf[sLen] = '\0';
    Serial.printf("Sonos UPnP -> %u.%u.%u.%u:%d  %s#%s\n", ip[0], ip[1], ip[2], ip[3], UPNP_PORT, serviceBuf, actionBuf);
  }
...
{
  size_t dataLen = strlen_P(data_P);
  size_t dataPos = 0;
  while (dataLen > dataPos)
  {
    size_t chunk = dataLen - dataPos;
    if (chunk > bufferSize - 1) chunk = bufferSize - 1;
    memcpy(buffer, (const char*)data_P + dataPos, chunk);
    buffer[chunk] = '\0';
